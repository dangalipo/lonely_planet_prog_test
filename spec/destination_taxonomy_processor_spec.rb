...
describe DestinationTaxonomyProcessor do
  ...
  describe "process" do
    ...
    context "taxonomy file is valid XML" do
    ...
    context "taxonomy file is not valid XML" do

      let(:filename) { File.join(Dir.pwd, "spec", "support", "invalid_xml.xml") }

      specify { expect{process}.to raise_error(InvalidTaxonomy, "Content file XML could not be parsed") }
